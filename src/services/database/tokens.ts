...
class TokenStore {
  ...
  constructor() {
    logger.info("Initializing TokenStore...");
  ...
  async loadToken(teamId: string): Promise<TokenData | undefined> {
    try {
      ...
      if (tokenData) {
        logger.info(`Token for team ${teamId} loaded from cache.`);
      } else {
        try {
          const encryptedData = await fs.readFile(
            this.getTokenPath(teamId),
            "utf-8",
          );
          ...
          logger.info(`Token for team ${teamId} loaded from file.`);
        } catch (fileError) {
          if ((fileError as NodeJS.ErrnoException).code !== "ENOENT") {
            logger.error(
              `Error reading token file for team ${teamId}:`,
              fileError,
            );
          }
          return undefined;
