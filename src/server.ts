...
app.post("/api/slack-events", async (req, res, next) => {
  if (req.body?.type === "url_verification") {
    return res.status(200).json({ challenge: req.body.challenge });
  }

  try {
    await slack.getRequestHandler()(req, res);
  } catch (error) {
    next(error);
  }
});

app.get("/api/slack-oauth", async (req, res, next) => {
  try {
    ...
    if (!data.ok) {
      throw new AppError(500, `Slack API error: ${data.error}`);
    }
